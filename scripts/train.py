...
if __name__ == '__main__':
    ...
    parser.add_argument('--source_func_path', default=str(DATA_PROCESSED_DIR / 'id2sourcecode'))
    ...
    graph1 = extract.add_nodes(G1, str(DATA_PROCESSED_DIR / 'id2sourcecode'))
    # graph1 = extract.add_nodes(G1, str(DATA_PROCESSED_DIR / 'filtered_code_files'))

    graph2 = metrics.add_nodes(G2, str(DATA_PROCESSED_DIR / 'id2sourcecode'))
    # graph2 = metrics.add_nodes(G2, str(DATA_PROCESSED_DIR / 'filtered_code_files'))
    ...
    #     max_workers=11)
    # task_list = [
    #     executor.submit(extract.add_nodes, G1, str(DATA_PROCESSED_DIR / 'id2sourcecode')),
    #     executor.submit(metrics.add_nodes, G3, str(DATA_PROCESSED_DIR / 'id2sourcecode'))
    # ]
    # process_results = [task.result() for task in as_completed(task_list)]
    ...
    output_dir = GENERATED_DIR / 'test1' / f'dim_{args.embed_dim}'
    output_dir.mkdir(parents=True, exist_ok=True)
    g2v_1.save(os.path.join(str(output_dir), "embedding"))
