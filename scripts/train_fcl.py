...
def datavec_dir(input_dim: int, split: str) -> str:
    return str(GENERATED_DIR / 'datavec' / f'dim_{input_dim}' / split)


def output_dir(layer_name: str, input_dim: int) -> Path:
    path = GENERATED_DIR / 'output' / layer_name / f'dim_{input_dim}'
...
def train_1(input_dim, model):
    ...
    vectors_dir = datavec_dir(input_dim, 'trainvec')
    ...
    for epoch in range(epochs):
        ...
        with open(output_dir('1_layers', input_dim) / 'train_result.txt', 'a+') as ff:
        ...
        # save the model
        torch.save(model.state_dict(), output_dir('1_layers', input_dim) / f'model_{epoch + 1}.pth')
...
def train_2(input_dim, model):
    ...
    for epoch in range(epochs):
        ...
        print(f'Training Time: {time.time() - begin_time}s')
        total_time += time.time() - begin_time
        with open(output_dir('2_layers', input_dim) / 'train_result.txt', 'a+') as ff:
            ff.write(f'\nModel structure: {model}\n')
            ff.write(f'Dim: {input_dim}\n')
            ff.write(f'Training Time: {time.time() - begin_time}s\n')
            ff.write('Training Loss: {:.4f}, Training Accuracy: {:.4f}\n\n'.format(np.mean(loss_history), (
        ...
        # save the model
        torch.save(model.state_dict(), output_dir('2_layers', input_dim) / f'model_{epoch + 1}.pth')
        print(f"current epoch: {epoch + 1}, time: {time.time() - begin_time}s")
...
def train_3(input_dim, model):
    ...
    for epoch in range(epochs):
        ...
        print(f'Training Time: {time.time() - begin_time}s')
        total_time += time.time() - begin_time
        with open(output_dir('3_layers', input_dim) / 'train_result.txt', 'a+') as ff:
            ff.write(f'\nModel structure: {model}\n')
            ff.write(f'Dim: {input_dim}\n')
            ff.write(f'Training Time: {time.time() - begin_time}s\n')
            ff.write('Training Loss: {:.4f}, Training Accuracy: {:.4f}\n\n'.format(np.mean(loss_history), (
        ...
        # save the model
        torch.save(model.state_dict(), output_dir('3_layers', input_dim) / f'model_{epoch + 1}.pth')
        print(f"current epoch: {epoch + 1}, time: {time.time() - begin_time}s")
...
def test_1(input_dim, model_load):
    ...
    test_vectors_dir = datavec_dir(input_dim, 'testvec')
    ...
    model_load.load_state_dict(torch.load(output_dir('1_layers', input_dim) / 'model_10.pth'))
    ...
    with open(output_dir('1_layers', input_dim) / 'test_result.txt', 'a+') as ff:
        ff.write(f'\nDim: {input_dim}\n')
...
def test_2(input_dim, model_load):
    ...
    # model_load = CodeCloneDetector(input_dim)
    # model_load = CodeCloneDetector(input_dim).to(device)  # GPU
    model_load.load_state_dict(torch.load(output_dir('2_layers', input_dim) / 'model_10.pth'))
    model_load.eval()
    ...
    with open(output_dir('2_layers', input_dim) / 'test_result.txt', 'a+') as ff:
        ff.write(f'\nDim: {input_dim}\n')
        ff.write(f'test_time: {end_time - start_time}s\n')
        ff.write(f'Test Accuracy: {accuracy:.2f}\n')
        ff.write(f'Test Precision: {precision:.2f}\n')
...
def test_3(input_dim, model_load):
    ...
    # model_load = CodeCloneDetector(input_dim)
    # model_load = CodeCloneDetector(input_dim).to(device)  # GPU
    model_load.load_state_dict(torch.load(output_dir('3_layers', input_dim) / 'model_10.pth'))
    model_load.eval()
    ...
    with open(output_dir('3_layers', input_dim) / 'test_result.txt', 'a+') as ff:
        ff.write(f'\nDim: {input_dim}\n')
        ff.write(f'test_time: {end_time - start_time}s\n')
        ff.write(f'Test Accuracy: {accuracy:.2f}\n')
        ff.write(f'Test Precision: {precision:.2f}\n')
